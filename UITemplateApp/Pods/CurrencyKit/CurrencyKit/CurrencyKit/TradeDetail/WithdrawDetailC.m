...
@end
@implementation WithdrawDetailC
...
- (void)updateWithdrawDetail {
    ...
    if (_currentPage <= 1) {
        ...
        _withdrawList = ccs.mutArray;
    }
    
    [ccs.accountLib_withdraw withdrawApplyPageQueryWithCurrentPage:_currentPage endDate:_endDate pageSize:20 startDate:_startDate success:^(HttpModel * _Nonnull result) {
        
        [self.withdrawList addObjectsFromArray:result.resultDic[@"withdrawApplySimples"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WithdrawDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil) {
        cell = [[WithdrawDetailTableViewCell alloc]
    ...
    [cell update:_withdrawList[indexPath.row]];
